...
function init() {
    ...
    $.ajax({
        ...
        success: function (data) {
            L.tileLayer(`https://t0.tianditu.gov.cn/img_w/wmts?layer=img&style=default&tilematrixset=w&Service=WMTS&Request=GetTile&Version=1.0.0&Format=tiles&TileMatrix={z}&TileCol={x}&TileRow={y}&tk=${data.token.tianditu}`, {
                ...
                //zoomOffset: 1,
                //tileSize: 256,
                attribution: 'Map data © <a href="https://www.tianditu.gov.cn">天地图</a>, <a>审图号：GS（2024）0568号</a>',
            }).addTo(map);
            L.tileLayer(`https://t0.tianditu.gov.cn/cia_w/wmts?layer=cia&style=default&tilematrixset=w&Service=WMTS&Request=GetTile&Version=1.0.0&Format=tiles&TileMatrix={z}&TileCol={x}&TileRow={y}&tk=${data.token.tianditu}`, {
                maxZoom: 18,
                attribution: 'Map data © <a href="https://www.tianditu.gov.cn">天地图</a>, <a>审图号：GS（2024）0568号</a>',
            }).addTo(map);
    ...
    if (localStorage.getItem("show-pilot") == "false") {
        $("#show-pilot").prop("checked", true);
    }
    if (localStorage.getItem("show-atc") == "false") {
        $("#show-atc").prop("checked", true);
    }
    if (localStorage.getItem("show-range") == "false") {
        $("#show-range").prop("checked", true);
    }
    ...
    map.on('popupopen', function(e) {
        ...
        if (d.type == "PILOT") detailDOM.html(`<div id=${d.callsign}>
                                                ...
                                                <img class="detail-img" onerror=src="" src="./image/airlines/${d.callsign.substring(0,3)}.png" />
                                                </div>
...
function updMap() {
    $.ajax({
        url: "http://test1.linuschen.ink/data.php",
        success: function (data) {
            let n = data.split("\n");
            n.pop();
            if (n[0] == "::::::::0::::0:FSD::" && player.length == 0) return;
    ...
    for (let i = 0; i < player.length; i++) {
        ...
        if (d.marker == null) {
            if (d.type == "ATC" && d.lat && d.lng) {
                ...
                player[i].tooltip = "";
                if (localStorage.getItem("tag-icon") == "true") {
                    player[i].tooltip += `<img class="tag-icon" onerror=src="" src="./image/airlines/${d.callsign.substring(0,3)}.png"></img><br>`;
                }
                if (localStorage.getItem("tag-callsign") == "true"){
